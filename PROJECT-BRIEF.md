...
NOTE current_build :: RESOLVED 2026-08-02 — build 14 is the first build ever to carry sharing, lendable and the borrow loop, and the two contradictory readings were BOTH correct. Per docs/TESTFLIGHT.md:106 internal testers skip Beta App Review, so Tate had build 14 thirty-one minutes after the cut and thirty-one minutes BEFORE it was submitted for review — which is what he saw in App Store Connect and why he fired friend_closet_opened at 01:12:07Z. The six are EXTERNAL testers on First Serve and are gated on that review. Tate has it; the cohort does not
...
NOTE gate_c_build :: no cohort member has ever fired friend_closet_opened, item_shared or lendable_set — all 16, 13 and 8 of those events belong to Tate. That is consistent with all six being on build 13 and is NOT evidence that anyone saw lending and declined it. No event carries app version, so this is inference from event availability, not a version read. Someone should confirm build 14's review state in App Store Connect directly; no ASC credential exists on this machine
